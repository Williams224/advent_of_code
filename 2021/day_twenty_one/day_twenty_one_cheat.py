    ...
    if player_1_turn:
        if rolls > 0:
            for roll in range(1, 4):
                res += count_player_1_wins(
                    score_1, score_2, find_new_pos(pos_1, roll), pos_2, rolls - 1, True
                )
        else:
            res += count_player_1_wins(score_1 + pos_1, score_2, pos_1, pos_2, 3, False)

    else:
        if rolls > 0:
            for roll in range(1, 4):
                res += count_player_1_wins(
                    score_1, score_2, pos_1, find_new_pos(pos_2, roll), rolls - 1, False
                )
        else:
            res += count_player_1_wins(score_1, score_2 + pos_2, pos_1, pos_2, 3, True)
...
def solve_part_2(pos_1, pos_2):
    return max(
        count_player_1_wins(0, 0, pos_1, pos_2, 3, True),
        count_player_1_wins(0, 0, pos_2, pos_1, 3, False),
    )


if __name__ == "__main__":
    print(solve_part_2(5, 10))